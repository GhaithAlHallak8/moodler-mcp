...
| `MOODLER_EMBED_FILES`           | ❌       | `local`  | When `download_resource` attaches the file itself as an embedded resource: `local` (Claude Code, Codex, ChatGPT desktop), `always`, or `never`. Claude Desktop and claude.ai reject attached files, so they never receive one regardless of this setting. Files over 700KB are never embedded. |
